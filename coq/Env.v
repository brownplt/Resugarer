...
Module ENV (Import Value : VALUE).
  ...
  (* Disjoint union *)
  Fixpoint union (e1 e2 : env) {struct e1} : option env :=
    match e1 with
      | mtEnv => Some e2
      | bind v t e1 =>
        if bmem v e2
          then None
          else match union e1 e2 with
                 | None => None
...
(*******************)
  ...
  Lemma mem__mem_bind : forall (v v' : var) (t : val) (e : env),
    mem v e -> mem v (bind v' t e).
  Proof.
    intros.
    unfold mem, bmem, lookup in *.
    destruct (beq_var v v'); auto.
  Qed.

  Lemma mem_bind : forall v t e, mem v (bind v t e).
  Proof.
    intros. unfold mem, bmem, lookup.
...
(**************************)
  ...
  Proof.
    ...
    exists x; split; auto. apply mem__mem_bind; auto.
  Qed.
  ...
  Proof.
    ...
    Case "bind v1 t e1".
      assert (~ mem v e1). intro; apply H0. apply mem__mem_bind; auto.
